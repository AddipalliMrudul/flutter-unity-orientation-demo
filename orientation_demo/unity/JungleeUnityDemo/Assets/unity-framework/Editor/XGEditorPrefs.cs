...
namespace XcelerateGames.Editor
{
    ...
    public class XGEditorPrefs
    {
        ...
        /// </summary>
        static XGEditorPrefs()
        {
            mAppKey = Utilities.MD5Hash(Application.dataPath) + "-";
        }
        ...
        {
            SetString(key, "#" + ColorUtility.ToHtmlStringRGBA(color));
        }

        /// <summary>
        /// Sets the value of the preference identified by key as a Color.
        /// </summary>
        /// <param name="key">key to use</param>
        /// <param name="value">value to save</param>
        public static Color GetColor(string key, Color color)
        {
            if(HasKey(key))
            {
                if (ColorUtility.TryParseHtmlString(GetString(key), out color))
                    return color;
                else
                    XDebug.LogError("Failed to parse Color");
            }
            return color;
        }
