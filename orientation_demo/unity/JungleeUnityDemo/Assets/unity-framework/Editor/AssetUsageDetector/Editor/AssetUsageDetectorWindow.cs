...
namespace AssetUsageDetectorNamespace
{
	...
	public enum AssetTypeToFind { Image, Material, Font }
...
#pragma warning restore 0649
		...
		private AssetTypeToFind assetTypeToFind = AssetTypeToFind.Image;
		...
		private void OnGUI()
		{
			...
			else if (currentPhase == Phase.Setup)
			{
				...
				assetTypeToFind = (AssetTypeToFind)EditorGUILayout.EnumPopup("", assetTypeToFind);
		...
		private void SearchAllAssetType()
		{
			currentPhase = Phase.Processing;
			objectsToSearch.Clear();
			string[] guids = AssetDatabase.FindAssets(GetAssetType());
			foreach (string guid in guids)
			{
				string path = AssetDatabase.GUIDToAssetPath(guid);
				Texture2D texture = AssetDatabase.LoadAssetAtPath<Texture2D>(path);
				objectsToSearch.Add(new ObjectToSearch(texture));
			}
			InitiateSearch();
		}
		...
		{
			return assetTypeToFind switch
			{
				AssetTypeToFind.Image => "t:Texture2D",
				AssetTypeToFind.Material => "t:Material",
				AssetTypeToFind.Font => "t:Font",
				_ => "t:Texture2D",
			};
		}
