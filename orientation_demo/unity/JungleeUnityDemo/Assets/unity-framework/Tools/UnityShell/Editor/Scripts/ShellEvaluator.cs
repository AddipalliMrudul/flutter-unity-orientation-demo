...
namespace XcelerateGames.EditorTools
{
    ...
    public class ShellEvaluator
    {
            ...
            new Thread(() =>
            {
                ...
                if (!string.IsNullOrEmpty(input))
                {
                    ...
                    if (handle == handleCount)
                    {
                        ...
                        for (var i = 0; i < completions.Length; i++) completions[i] = input + completions[i];
        ...
        public object Evaluate(string command)
        {
			...
			var compilationResult = Evaluator.Compile(command);
#endif
            if (compilationResult == null) return "command not found";

            object result = null;
            compilationResult(ref result);

            if (result == null) result = "executed successfully";
            return result;
        }
        #endregion//============================================================[ Public ]
